...
import type { Approval, ApprovalRequestType } from "@/types";
...
export default function ApprovalsPanel({ eventId }: Props) {
  ...
  const fetchApprovals = useCallback(async () => {
    try {
      const data = await listApprovals(eventId);
  ...
  const handleApprove = async (approval: Approval) => {
    ...
    try {
      await reviewApproval(eventId, approval.id, "approved");
  ...
  const handleRejectSubmit = async (approval: Approval) => {
    if (!rejectNotes.trim()) {
    ...
    setActionLoading(approval.id);
    try {
      await reviewApproval(eventId, approval.id, "rejected", rejectNotes);
      toast.success("Rejected. Re-running with your feedback…");
      setRejectingId(null);
      setRejectNotes("");
      await fetchApprovals();

      // Auto re-trigger based on type
      ...
      } else if (approval.request_type === "judge_assignment") {
        const res = await autoAssignJudges(eventId, 2);
        toast.success(res.message || "New judge assignment proposal created.");
      }
