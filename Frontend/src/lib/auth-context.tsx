...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Sync organizer profile via Firebase token → backend /auth/login → EKAM JWT
  ...
  // Exposed so pages can force a re-fetch after login
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        // Organizer / Admin path — exchange Firebase token for EKAM JWT
